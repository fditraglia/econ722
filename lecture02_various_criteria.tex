...
In the preceding argument I claimed that the influence function for MLE is given by
	$$\mbox{infl}(G,y) = J^{-1} \left(\frac{\partial \log f(y|\theta_0)}{\partial \theta}\right)$$
Here's a proof of this statement, following Chapter 5 of Konishi \& Kitagawa (2008).\footnote{Many thanks to Laura Liu for working out the details of this argument and typing it up for me!} First, note that the functional $\mathbb{T}$ for MLE can be (implicitly) defined in terms of the score by
...
\end{eqnarray*}
Now we'll differentiate each piece. By the product rule and the chain rule
	\begin{eqnarray*}
		\frac{\partial}{\partial \epsilon} A(\epsilon) &=& -A(\epsilon) + (1-\epsilon) \frac{\partial}{\partial \epsilon} A(\epsilon)\\
			&=& -A(\epsilon) + (1-\epsilon) \int \frac{\partial}{\partial \epsilon}  s(z|\theta(\epsilon,y))\; dG(z)\\
			&=& -A(\epsilon) + (1-\epsilon) \int \frac{\partial s(z|\theta(\epsilon,y))}{\partial \theta} \frac{\partial \theta(\epsilon,y)}{\partial \epsilon}\; dG(z)
	\end{eqnarray*}
and similarly
	\begin{eqnarray*}
		\frac{\partial}{\partial \epsilon} B(\epsilon) &=& B(\epsilon) +\epsilon\; \frac{\partial}{\partial \epsilon} B(\epsilon)\\
		&=&B(\epsilon) + \epsilon \; \frac{\partial s(y|\theta(\epsilon,y))}{\partial \theta} \frac{\partial \theta(\epsilon,y)}{\partial \epsilon}
	\end{eqnarray*}
Now, we're going to evaluate the derivative at $\epsilon = 0$, so we have
	\begin{eqnarray*}
		\frac{\partial}{\partial \epsilon}\left[\int s(z|\theta(\epsilon,y))\; dH(z) \right]_{\epsilon = 0} &=& B(\epsilon) - A(\epsilon)  + \int \frac{\partial s(z|\theta(\epsilon,y))}{\partial \theta} \frac{\partial \theta(\epsilon,y)}{\partial \epsilon}\; dG(z)
	\end{eqnarray*}

\begin{align}
 & \int\left.\frac{\partial\log f\left(z|\theta\right)}{\partial\theta}\right|_{\theta=\mathbb{T}\left(G\right)}d\left(\delta_{y}\left(z\right)-G\left(z\right)\right)\nonumber \\
 & +\int\left.\frac{\partial\log f\left(z|\theta\right)}{\partial\theta\partial\theta^{\prime}}\right|_{\theta=\mathbb{T}\left(G\right)}dG\left(z\right)\cdot\frac{\partial}{\partial\epsilon}\left[\mathbb{T}\left(\left(1-\epsilon\right)G+\epsilon\delta_{y}\right)\right]=0\label{eq:derivative}
\end{align}

To ensure the uniqueness of $\frac{\partial}{\partial\epsilon}\left[\mathbb{T}\left(\left(1-\epsilon\right)G+\epsilon\delta_{y}\right)\right]$,
we need the following condition (equation 5.17 in the book)
\[
\int\frac{\partial}{\partial\epsilon}\left[\mathbb{T}\left(\left(1-\epsilon\right)G+\epsilon\delta_{y}\right)\right]dG\left(z\right)=0
\]
So the first term in equation (\ref{eq:derivative}) becomes
\begin{align}
\int\left.\frac{\partial\log f\left(z|\theta\right)}{\partial\theta}\right|_{\theta=\mathbb{T}\left(G\right)}d\left(\delta_{y}\left(z\right)-G\left(z\right)\right) & =\int\left.\frac{\partial\log f\left(z|\theta\right)}{\partial\theta}\right|_{\theta=\mathbb{T}\left(G\right)}d\delta_{y}\left(z\right)\nonumber \\
 & =\left.\frac{\partial\log f\left(y|\theta\right)}{\partial\theta}\right|_{\theta=\mathbb{T}\left(G\right)}\label{eq:1st_term}
\end{align}

Thus,
\begin{align*}
\mathrm{infl\left(G,y\right)} & =\lim_{\epsilon\rightarrow0}\frac{\mathbb{T}\left(\left(1-\epsilon\right)G+\epsilon\delta_{y}\right)-\mathbb{T}\left(G\right)}{\epsilon}\\
 & =\frac{\partial}{\partial\epsilon}\left[\mathbb{T}\left(\left(1-\epsilon\right)G+\epsilon\delta_{y}\right)\right]\\
...
\end{align*}
where the third line is given by equations (\ref{eq:derivative})
and (\ref{eq:1st_term}), and the fourth line follows the definitions
of $J$ and $\theta_{0}$.
