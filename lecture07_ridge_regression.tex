...
where $\widehat{\beta}$ is the OLS estimator and $(x)_+ = \max(x,0)$. To make this operational, we substitute an estimator of $\sigma^2$, for example one based on the OLS residuals. 

Now that we know there is at least one estimator that can beat OLS, we'll consider the idea of shrinkage more generally, starting with Ridge Regression.
...
\todo[inline]{Data-dependent mapping with ``fitting budget.''}
...
\todo[inline]{Explain how $\lambda$ controls both, using the preceding formula. Makes it very clear how the bias arises and why the variance is lower. Compare to OLS formulas.}
