...
The next step in the derivation of the FIC is to work out the limiting distribution of the ML estimators $(\widehat{\theta}_S, \widehat{\gamma}_S)$ under model $S$. This result appears as Lemma 3.2 in Hjort \& Claeskens (2003).
\begin{lem}
Under local mis-specification,
$$\left[\begin{array}{c}
...
To make the final results a bit more compact, Hjort \& Claeskens (2003) introduces the following notation
	\begin{eqnarray*}
		W &\equiv& J^{10}M + J^{11} N\\
		\widehat{\delta}_S &\equiv& \sqrt{n}\left(\widehat{\gamma}_{S} -\gamma_{0,S} \right)\\
		D_S &\equiv& K_S \pi_S K^{-1}(\delta + W) 
	\end{eqnarray*}
The random variable $W$	is simply a linear combination of the random variables $M$ and $N$ that emerged from applying a CLT to the score of the full model. It's worth giving a name to this quantity because of the following result. Note that this doesn't actually appear as a lemma in the paper: it's one of those ``it's not difficult to show'' assertions and appears immediately after Lemma 3.2. In any case, here is the result and the proof:
\begin{lem}
Define $W\equiv J^{10}M+J^{11}N$. Then, $W= K(N - J_{10}J_{00}^{-1}M)$ and $M$ and $W$ are indep.\ with $W\sim \mathcal{N}_q(0,K)$ and $M\sim\mathcal{N}_p(0,J_{00})$.
\end{lem}
...
\end{eqnarray*}
where the first equality uses the fact that $J_{00}J^{01}+J_{01}J^{11} =0$. The result follows.
\end{proof}
...
\subsection{Write Out the FIC Formula}

\subsection{Give a Simple Example}
