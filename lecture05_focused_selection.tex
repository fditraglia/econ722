...
So what can we do about $\delta$? Notice from above that we actually need to estimate $\delta \delta'$, \emph{not} $\delta$. If we had a consistent estimator $\widetilde{\delta}$ of $\delta$, then $\widetilde{\delta}\widetilde{\delta}'$ would be a consistent estimator of $\delta\delta'$. Unfortunately \emph{no consistent estimator of $\delta$ exists under local mis-specification}. Intuitively, the problem is that the data become ``less and less informative'' about $\delta$ as the sample size grows. Instead, the FIC substitutes an \textbf{asymptotically unbiased estimator} of this quantity, constructed as follows. First, we know from  Lemma \ref{lem:W} that 
$$D_n = \widehat{\delta}_{Full} \overset{d}{\rightarrow} D = \delta + W \sim \mbox{N}_q(\delta,K)$$
Thus, $\widehat{\delta}_{Full}$ is an \emph{asymptotically unbiased estimator of} $\delta$. By the Continuous Mapping Theorem,
	$$D_nD_n' \overset{d}{\rightarrow} D D'$$
But, by the shortcut formula
	$$E[DD'] = Var(D) + E[D]E[D'] = K + \delta \delta'$$
which means that $D_nD_n'$ is an asymptotically \emph{biased} estimator of $\delta\delta'$. Fortunately, to remove the bias we simply need to subtract $K$. Thus, our asymptotically unbiased estimator of $\delta \delta'$ is 
	$$D_nD_n' - \widehat{K}$$
...
The FIC, however, is expressed in a slightly different and simpler way by removing constants that do not vary across models. First, we construct the \emph{limit experiment} version of the AMSE substitutes $DD' - K$ for $\delta\delta'$, yielding
\begin{eqnarray*}
...
\end{eqnarray*}
The limiting (i.e.\ infeasible) version of the FIC simply subtracts $\tau_0^2 - \omega'K\omega$ since this is constant across models, yielding
	\begin{eqnarray*}
		FIC &=&  \omega' (I - K^{1/2}H_SK^{-1/2})DD'(I - K^{-1/2}H_SK^{1/2})\omega +
	\end{eqnarray*}


which is an unbiased estimator of the limiting risk.

where we have ignored the term $\tau_0^2$ since it does not vary across models.
...
Thus, our asymptotically unbiased estimator of $\delta \delta'$ is 
	$$D_nD_n' - \widehat{K}$$
Substituting this into the expression for AMSE, along with consistent estimators of all other quantities, gives the FIC:
	$$FIC(S) =\omega' (I - K^{1/2}H_SK^{-1/2})\delta \delta'(I - K^{1/2}H_SK^{-1/2})\omega + \omega'K^{1/2}H_S K^{1/2}\omega $$
where we have ignored the term $\tau_0^2$ since it does not vary across models.
