...
\todo[inline]{Exchanging integral and derivative in the objective function, deal with terms one at a time, $f$ doesn't depend on paramters, etc...}
...
Differentiating, we have
	...
	\end{eqnarray*}
where we have used the fact that each term is a scalar, and thus equals its trace, and $\mbox{tr}(AB) = \mbox{tr}(BA)$ with $\mathbf{z}_t$ playing the role of $A$ for each of the two terms. It remains to calculate two matrix derivatives. For the first term we need to calculate $\nabla_A \mbox{tr}(AB)$ where $\Lambda'$ plays the role of $A$ and $\Psi^{-1}(\mathbf{x}_t = \mu)\mathbf{z}_t'$ plats the role of $B$. 

For the second term we need to calculate $\nabla_A \mbox{tr}(ABA'C)$ where $\Lambda'$ plays the role of $A$
