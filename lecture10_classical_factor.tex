...
\paragraph{Updating $\mu$:} This is by far the easiest step since it turns out not to depend on $j$!
...
Differentiating:
 \begin{eqnarray*}
 	\nabla_\mu \log p(\mathbf{x}_t)|\mathbf{z}_t;\mu, \Lambda, \Psi) &=& -\frac{1}{2} \left( -\mu' \Psi^{-1} \mathbf{x}_t + \mu' \Psi^{-1}\mu + \mu' \Psi^{-1} \Lambda \mathbf{z}_t - \mathbf{x}_t' \Psi^{-1} \mu +  \mathbf{z}_t'\Lambda' \Psi^{-1} \mu  \right)\\
 		&=&
 \end{eqnarray*}
