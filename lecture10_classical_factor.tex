...
\subsection{Multivariate Normal: Marginals and Conditionals}
...
Classical Factor Analysis specifies a joint distribution on the observable random $p$-vector $X$ and an unobserved or ``latent'' random $k$-vector $Z$, namely
	\begin{eqnarray*}
		Z &\sim& N_k(0_k, I_k)\\
		X|Z &\sim& N_p(\mu + \Lambda Z, \Psi)
	\end{eqnarray*}
where $\mu$ is a $p\times 1$ vector of parameters, $\Lambda$ is a $p\times k$ matrix of parameters and $\Psi$ is a $p\times p$ \emph{diagonal} matrix of parameters. The idea is that $k$ is strictly less than both $p$, the dimension of $X$, and $n$, the sample size. 

An equivalent way to define the classical factor model is as follows:
	\begin{eqnarray*}
		Z &\sim& N_k(0_k, I_k)\\
