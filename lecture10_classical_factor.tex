...
\paragraph{Updating $\mu$:} By this point you might have guessed that, since $\bar{x}$ is the MLE for the mean of a normal distribution, we could have simply de-meaned each of the $\bf{x}_t$ before proceeding, thus saving ourselves the trouble of carrying around the parameter vector $\mu$. This turns out to be correct, but for completeness we'll work out the M-step for $\mu$ in full. 
...
Left-multiplying both sides by $\Psi$, using the fact that $f_t^{(j-1)}(\mathbf{z}_t)$ is a density, and substituting the E-step gives
 ...
 T\bar{x} - \Lambda \sum_{t = 1}^T \int f_t^{(j-1)}(\mathbf{z}_t) \; \mathbf{z}_t \; d\mathbf{z}_t &=& T \mu \\
 	\bar{x}  - \Lambda \left( \frac{1}{T} \sum_{t=1}^T \mu^{(j-1)}_{\mathbf{z}_t|\mathbf{x}_t} \right) &=& \mu
	\end{eqnarray*}
