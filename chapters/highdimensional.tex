...
\todo[inline]{Try to give the intuition for what's going on here and be clear about why the $p\geq 3$ requirement is possibly a little strange for the typical estimation problem in economics but is very sensible for forecasting. Explain that James-Stein itself is inadmissible and explain about the positive-part version that avoids shrinking ``past'' zero. (Even that one isn't admissible though!) Explain how we could shrink towards any value we like, possibly something suggested by economic theory, and give the general version for regression. Give some references to Hansen stuff etc.} 
...
\subsection{The James-Stein Estimator (OLD VERSION)}
It turns out that OLS is inadmissible under squared error loss when $p\geq 3$.
This result was so surprising when first discovered that it is sometimes called ``Stein's Paradox.''
Efron and Morris (1977) provide a nice discussion as does Samworth (????). 
An estimator that can be shown to dominate OLS is the so-called ``positive-part James-Stein estimator'' which is given by
	$$\widehat{\beta}^{JS} = \widehat{\beta}\left[1 -\frac{(p-2)\sigma^2}{\widehat{\beta}' \widehat{\beta}} \right]_+$$
where $\widehat{\beta}$ is the OLS estimator and $(x)_+ = \max(x,0)$.
To make this operational, we substitute an estimator of $\sigma^2$, for example one based on the OLS residuals. 
We see that this estimator shrinks us \emph{away} from the least-squares fit and towards zero. 
