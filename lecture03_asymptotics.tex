...
\todo[inline]{If there's time, write out the LR statistic for this case: it's very easy.}
...
where
	\begin{eqnarray*}
		...
		b &=& \sqrt{d_T} - \sqrt{T}\mu
	\end{eqnarray*}
\todo[inline]{Make a Shiny App of this for AIC versus BIC if there's time. Explain how BIC has max risk that diverges in exchange for consistency at the null model. Also this bad behavior occurs very near to the point where the BIC is supposedly achieving its advantage. In constrast, max risk of AIC is bounded.  Talk about pointwise versus uniform results.}

\section{Efficient Model Selection}
Motivate from behavior of AIC in preceding example. 
