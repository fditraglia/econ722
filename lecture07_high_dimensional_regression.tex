...
There is another kind of shrinkage estimation that is very closely related to Ridge Regression called \textbf{principal components regression} (PCR). The procedure is very simple:
	\begin{enumerate}
		\item Calculate the principal components $\mathbf{z}_1, \hdots, \mathbf{z}_p$ of the design matrix $X$.
		\item Throw away all but the first $k$ principal components, where $k <p$.
		\item Regress \textbf{y} on $\mathbf{z_1}, \hdots, \mathbf{z}_k$ using OLS. 
	\end{enumerate}
