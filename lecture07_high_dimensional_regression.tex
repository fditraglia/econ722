...
By analogy, we could imagine trying some \emph{other} penalty function to get a different kind of shrinkage behavior. The Least Absolute Shrinkage and Selection Operator (LASSO) does exactly this by adding an $L_1$ penalty to the OLS criterion function:
...
\section{What about Inference?}
Could go Bayesian. For a Frequentist, this is a very hard problem. A couple of recent papers, including Belloni et al.

\section{Computations Using R}
Chapter 6 of James et al.\ (2013) ends with three ``Labs''illustrating how to carry out various model selection and shrinkage procedures in R. The second and third of these illustrate Ridge Regression, Lasso, and PCR. There are some errors in the code as it appears in the book, so I've put together a corrected version called \texttt{ISLR\_ch6\_lab\_.R} and posted it in the GitHub repository for this class, available at \url{https://github.com/fditraglia/econ722}. This code makes heavy use of the excellent \texttt{GLMNET} package for R which is documented in Friedman, Hastie \& Tibshirani (2010).   
\end{document}