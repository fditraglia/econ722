...
\begin{eqnarray*}
	(\mathbf{y}-X\beta)'(\mathbf{y}-X\beta) &=&
		\mathbf{y}'\mathbf{y} + \beta'X'X\beta -2\beta'X' \mathbf{y} \\
		&=& \mathbf{y}'\mathbf{y} +\beta'\beta -2\beta' X'\mathbf{y}\\
		&=& (\mbox{constant}) + \sum_{j=1}^p \beta_j^2 - 2 \beta'\left( \sum_{i=1}^n \mathbf{x}_i y_i\right)\\
		&=& (\mbox{constant}) + \sum_{j=1}^p \beta_j^2 - 2 \sum_{j=1}^p \sum_{i=1}^n \beta_j x_{ij}y_i\\
\end{eqnarray*}
Thanks to this convenient simplification, solving for the MLE is simple. The derivative of the criterion function with respect to $\beta_j$ is 
	$$2\beta_j  - 2 \sum_{i=1}^n x_{ij} y_i$$
so the MLE is
	$$\widehat{\beta}^{MLE}_j = \sum_{i=1}^n x_{ij}y_i$$
The solution to Ridge Regression is also very simple in the case of orthonormal regressors. We have:
	$$\widehat{\beta}_{Ridge} =\left(X'X + \lambda I_p \right)^{-1}X'y = \left(I_p + \lambda I_p \right)^{-1}\left( \sum_{i=1}^n \mathbf{x}_i y_i\right)$$
hence
	$$\widehat{\beta}^{Ridge}_j = \frac{1}{1+\lambda} \sum_{i=1}^n x_{ij}y_i = \left(\frac{1}{1+\lambda}\right)\widehat{\beta}^{MLE}_j$$
The calculations for Lasso are a bit more involved since there is no closed-form solution. 
