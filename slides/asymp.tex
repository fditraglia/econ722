...
\begin{frame}
  ...
  \begin{itemize}
    \item AIC and TIC \emph{cannot} satisfy (ii) since $(c_{T,\ell} - c_{T,j_0})$ \emph{does not depend on sample size}.
    \item  It turns out that AIC and TIC are \emph{not} consistent. 
...
\begin{frame}
  \frametitle{Efficiency}
  \begin{itemize}
    \item Roughly speaking, a model selection criterion is called efficient if it performs ``nearly as well'' as the theoretical optimum relative to some loss function. 
    \item More broadly, an efficient/conservative criterion is one that has ``good risk properties.''
